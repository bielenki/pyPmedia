...
class thiessenClip(QgsProcessingAlgorithm):
    ...
    def fThiessenClip(self, parameters, context, model_feedback):
        ...
        alg_params = {
            ...
            'BUFFER': 1000,
            'OUTPUT': 'memory:voronoipolygon' #parameters['layerThiessenTemp']
        }
        outputs['voronoi'] = processing.run('qgis:voronoipolygons', alg_params, context=None, feedback=None) # is_child_algorithm=True)
        '''
        # v.voronoi
        alg_params = {
            '-l': False,
